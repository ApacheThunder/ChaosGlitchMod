...
namespace ChaosGlitchMod.ChaosComponents {
    ...
    public class ChaosWallMimicManager : CustomEngageDoer, IPlaceConfigurable {
        
        public List<string> GlitchEnemyList;
        ...
        private bool m_GlitchModeActive;
        private bool m_ChaosModeActive;
        ...
        private float m_spawnGitchEnemyOdds;
        ...
        public ChaosWallMimicManager() {
            ...
            if (GlitchEnemyList == null) {
                GlitchEnemyList = new List<string> {
            ...
            m_AssetBundle = null;
            m_isHidden = true;
            m_GlitchModeActive = false;
            m_ChaosModeActive = false;
        ...
        public void Start() {
            ...
            if (ChaosConsole.GlitchEnemies | ChaosConsole.GlitchEverything) {
                if (UnityEngine.Random.value <= m_GlitchOdds) { m_GlitchModeActive = true; }
            }           
        ...
        private IEnumerator BecomeMimic() {            
            ...
            if (!ChaosConsole.WallMimicsUseRewardManager) { m_ChaosModeActive = true; }
            if (m_GlitchModeActive) { m_ItemDropOdds += 0.2f; m_FriendlyMimicOdds += 0.2f; }
            ...
            if (aiShooter) { aiShooter.ToggleGunAndHandRenderers(true, "ChaosWallMimicController"); }
        ...
        private IEnumerator DoIntro() {
            ...
            knockbackDoer.SetImmobile(true, "ChaosWallMimicController");
            m_hands = GetComponentsInChildren<GunHandController>();
            for (int i = 0; i < m_hands.Length; i++) { m_hands[i].gameObject.SetActive(false); }
            yield return null;
            aiActor.ToggleRenderers(false);
            if (aiShooter) { aiShooter.ToggleGunAndHandRenderers(false, "ChaosWallMimicController"); }
            SpeculativeRigidbody specRigidbody = this.specRigidbody;
