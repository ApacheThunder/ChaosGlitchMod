...
namespace ChaosGlitchMod
{
    class HooksAndLists : MonoBehaviour
    {
        ...
        public static Vector2 actorSize;
        ...
        public static void AwakeHookCustom(Action<AIActor> orig, AIActor self)
        {
            ...
            string skusketHeadGUID = "c2f902b7cbe745efb3db4399927eab34";
            ...
            if (skusketHeadGUID.Contains(self.EnemyGuid)) { self.DiesOnCollison = true; }
                ...
                if (UnityEngine.Random.value <= RandomResizedEnemies)
                {
                    if (!BannedEnemyGUIDList.Contains(self.EnemyGuid))
                    {
                        if (UnityEngine.Random.value <= RandomSizeChooser)
                        {
                            ...
                            if (DieOnCollisionIfTinyGUIDList.Contains(self.EnemyGuid)) {
                            ...
                            if (healthHaver.IsBoss) {
                                healthHaver.SetHealthMaximum(healthHaver.GetMaxHealth() / 1f, 1.5f, false);
                                self.BaseMovementSpeed *= 0.8f;
                                self.MovementSpeed *= 0.8f;
                            } else {
                                self.BaseMovementSpeed *= 0.75f;
                                self.MovementSpeed *= 0.75f;
                                healthHaver.SetHealthMaximum(healthHaver.GetMaxHealth() / 1f, 2f, false);
                            }
                        }
        ...
        public static void EnteredNewRoomHookForGlitch(Action<RoomHandler, PlayerController> orig, RoomHandler self, PlayerController p)
        {
            orig(self, p);
            room = self;
        ...
        public static void SpawnAnnoyingEnemy(Action<MinorBreakable> orig, MinorBreakable self)
        {
            ...
            // ETGModConsole.Log(self.name, false); RandomCritter

            string tombstonerEnmeyGUID = "cf27dd464a504a428d87a8b2560ad40a";
            ...
            AIActor TombstonerGUID = EnemyDatabase.GetOrLoadByGuid(tombstonerEnmeyGUID);

            // Checks if in combat and in room player is in before spawning something.
            if (primaryPlayer.IsInCombat && primaryPlayer.CurrentRoom == self.sprite.WorldCenter.GetAbsoluteRoom())
            {
                PotFairyEngageDoer.InstantSpawn = true;

                if (!flagSkull | !flagUrn | !flagWine | !flagArmor)
                {
                    if (UnityEngine.Random.value < 0.7) AIActor.Spawn(PotEnemyListNoFairies, self.sprite.WorldCenter, primaryPlayer.CurrentRoom, true, 0, true);
                }
                if (!flagPot)
                {
                    if (UnityEngine.Random.value < 0.7) AIActor.Spawn(PotEnemyList, self.sprite.WorldCenter, primaryPlayer.CurrentRoom, true, 0, true);
                }
                if (!flagTombstone)
                {
                ...
                {
                    if (UnityEngine.Random.value < 0.55) AIActor.Spawn(PestList, self.sprite.WorldCenter, primaryPlayer.CurrentRoom, true, 0, true);
                }
            }
            ...
            if (TrollModule.addRandomEnemy && primaryPlayer.IsInCombat && !BonusEnemyFlag)
            {
                ...
                {
                    AIActor aIActor = AIActor.Spawn(SelectedEnemy, randomAvailableCell.Value, GameManager.Instance.PrimaryPlayer.CurrentRoom, true, AIActor.AwakenAnimationType.Default, true);
